...
export function Client({ bot, store }) {
   ...
   const client = Object.defineProperties(bot, {
      getContentType: {
         value(content) {
            if (content) {
      ...
      getName: {
         value(jid) {
            ...
            if (id?.endsWith("@g.us")) return new Promise(async (resolve) => {
               v = bot.contacts[id] || bot.messages["status@broadcast"]?.array?.find(a => a?.key?.participant === id)
      ...
      sendContact: {
         async value(jid, number, quoted, options = {}) {
            ...
            for (let v of number) {
               list.push({
                  displayName: await bot.getName(v),
                  vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await bot.getName(v + "@s.whatsapp.net")}\nFN:${await bot.getName(v + "@s.whatsapp.net")}\nitem1.TEL;waid=${v}:${v}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:${config.Exif.packEmail}\nitem2.X-ABLabel:Email\nitem3.URL:${config.Exif.packWebsite}\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
               })
            }
            return bot.sendMessage(jid, {
               ...
               mentions: quoted?.participant ? [bot.decodeJid(quoted?.participant)] : [bot.decodeJid(bot?.user?.id)],
               ...options
            }, { quoted, ...options })
         },
         enumerable: true
      ...
      sendMedia: {
         async value(jid, url, quoted = "", options = {}) {
            ...
            if (size > 45000000) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${bot.user?.name} (${new Date()}).${ext}`, ...options }
            else if (options.asDocument) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${bot.user?.name} (${new Date()}).${ext}`, ...options }
            ...
            let msg = await bot.sendMessage(jid, data, { quoted, ...options })
            return msg
         },
         enumerable: true
      },
      ...
      cMod: {
         value(jid, copy, text = '', sender = bot.user.id, options = {}) {
            let mtype = bot.getContentType(copy.message)
            ...
            copy.key.fromMe = areJidsSameUser(sender, bot.user.id)
      ...
      sendGroupV4Invite: {
         async value(jid, groupJid, inviteCode, inviteExpiration, groupName, jpegThumbnail, caption = "Invitation to join my WhatsApp Group", options = {}) {
            const media = await prepareWAMessageMedia({ image: (await Function.getFile(jpegThumbnail)).data }, { upload: bot.waUploadToServer })
            ...
            const m = generateWAMessageFromContent(jid, message, { userJid: bot.user?.id })
            await bot.relayMessage(jid, m.message, { messageId: m.key.id })
...
export async function Serialize(bot, msg) {
   const m = {}
   ...
   if (msg.key) {
      m.key = msg.key
      m.from = bot.decodeJid(m.key.remoteJid)
      ...
      m.sender = bot.decodeJid(m.fromMe ? bot.user.id : m.isGroup ? m.participant : m.from)
   }

   m.pushName = msg.pushName
   m.isOwner = m.sender && [...config.options.owner, botNumber.split`@`[0]].includes(m.sender.replace(/\D+/g, ""))
   if (m.isGroup) {
      m.metadata = await bot.groupMetadata(m.from)
   ...
   if (m.message) {
      m.type = bot.getContentType(m.message) || Object.keys(m.message)[0]
      ...
      m.reply = async (text, options = {}) => {
         ...
         if ((Buffer.isBuffer(text) || /^data:.?\/.*?;base64,/i.test(text) || /^https?:\/\//.test(text) || fs.existsSync(text))) {
            ...
            if (!options.mimetype && (/utf-8|json/i.test(data.mime) || data.ext == ".bin" || !data.ext)) {
               ...
               return bot.sendMessage(chatId, { text, mentions: [m.sender, ...bot.parseMention(text)], ...options }, { quoted, ephemeralExpiration: m.expiration, ...options })
            } else {
               return bot.sendMedia(m.from, data.data, quoted, { ephemeralExpiration: m.expiration, ...options })
            }
         } else {
            if (!!config.msg[text]) text = config.msg[text]
            return bot.sendMessage(chatId, { text, mentions: [m.sender, ...bot.parseMention(text)], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
      ...
      m.download = (filepath) => {
         if (filepath) return bot.downloadMediaMessage(m, filepath)
         else return bot.downloadMediaMessage(m)
      }
   }
   ...
   if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
         m.quoted.type = bot.getContentType(m.quoted.message) || Object.keys(m.quoted.message)[0]
         m.quoted.msg = extractMessageContent(m.quoted.message[m.quoted.type]) || m.quoted.message[m.quoted.type]
         m.quoted.key = {
            remoteJid: m.msg?.contextInfo?.remoteJid || m.from,
            participant: m.msg?.contextInfo?.remoteJid?.endsWith("g.us") ? bot.decodeJid(m.msg?.contextInfo?.participant) : false,
         ...
         if (m.quoted.isGroup) {
            m.quoted.metadata = await bot.groupMetadata(m.quoted.from)
         ...
         m.quoted.download = (filepath) => {
            if (filepath) return bot.downloadMediaMessage(m.quoted, filepath)
            else return bot.downloadMediaMessage(m.quoted)
         }
      }
   }
